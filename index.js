...
function gerarFaturaHTML(fatura, pecas) {
  ...
  response += "<ul>\n";
  for (let apre of fatura.apresentacoes) {
    response += `<li> ${getPeca(pecas, apre).nome}: ${formatarMoeda(calcularTotalApresentacao(pecas, apre))} (${apre.audiencia} assentos) </li>\n`;
  }
  response += "</ul>\n";
  
  response +=  `<p> Valor total: ${formatarMoeda(calcularTotalFatura(pecas, fatura.apresentacoes))} </p>\n` +
